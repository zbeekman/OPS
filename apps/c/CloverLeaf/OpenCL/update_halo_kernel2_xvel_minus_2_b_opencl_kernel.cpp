...
void buildOpenCLKernels_update_halo_kernel2_xvel_minus_2_b(int xdim0,
  ...
  if (!isbuilt_update_halo_kernel2_xvel_minus_2_b) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_xvel_minus_2_b.cl"};
    ...
    printf("Compiling update_halo_kernel2_xvel_minus_2_b %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_xvel_minus_2_b=%d  "
                           "-Dxdim1_update_halo_kernel2_xvel_minus_2_b=%d ",
    ...
    OPS_opencl_core.kernel[58] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_xvel_minus_2_b", &ret);
    clSafeCall(ret);
...
void ops_par_loop_update_halo_kernel2_xvel_minus_2_b(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel2_xvel_minus_2_b(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[58], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[58], 2, sizeof(cl_int),
                            (void *)arg2.data));

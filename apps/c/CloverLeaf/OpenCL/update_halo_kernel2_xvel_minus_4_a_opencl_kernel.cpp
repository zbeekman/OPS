...
void buildOpenCLKernels_update_halo_kernel2_xvel_minus_4_a(int xdim0,
  ...
  if (!isbuilt_update_halo_kernel2_xvel_minus_4_a) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_xvel_minus_4_a.cl"};
    ...
    printf("Compiling update_halo_kernel2_xvel_minus_4_a %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_xvel_minus_4_a=%d  "
                           "-Dxdim1_update_halo_kernel2_xvel_minus_4_a=%d ",
    ...
    OPS_opencl_core.kernel[55] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_xvel_minus_4_a", &ret);
    clSafeCall(ret);
...
// host stub function
void ops_par_loop_update_halo_kernel2_xvel_minus_4_a(char const *name,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 3, range, 55))
    return;
#endif
  ...
  buildOpenCLKernels_update_halo_kernel2_xvel_minus_4_a(xdim0, xdim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[55], 6, sizeof(cl_int),
                            (void *)&y_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[55], 3, NULL,

...
inline void update_halo_kernel4_plus_2_back_gpu(double *vol_flux_y, double *mass_flux_y, const int* fields) {
...
int size2 ){
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * xdim0_update_halo_kernel4_plus_2_back + idx_z * 1*1 * xdim0_update_halo_kernel4_plus_2_back * ydim0_update_halo_kernel4_plus_2_back;
  arg1 += idx_x * 1*1 + idx_y * 1*1 * xdim1_update_halo_kernel4_plus_2_back + idx_z * 1*1 * xdim1_update_halo_kernel4_plus_2_back * ydim1_update_halo_kernel4_plus_2_back;

  if (idx_x < size0 && idx_y < size1 && idx_z < size2) {
    update_halo_kernel4_plus_2_back_gpu(arg0, arg1, &arg2);
  }
...
// host stub function
// host stub function
void ops_par_loop_update_halo_kernel4_plus_2_back_execute(ops_kernel_descriptor *desc) {
  #ifdef OPS_MPI
  ...
  if (xdim0 != xdim0_update_halo_kernel4_plus_2_back_h || ydim0 != ydim0_update_halo_kernel4_plus_2_back_h || xdim1 != xdim1_update_halo_kernel4_plus_2_back_h || ydim1 != ydim1_update_halo_kernel4_plus_2_back_h) {
    cudaMemcpyToSymbolAsync( xdim0_update_halo_kernel4_plus_2_back, &xdim0, sizeof(int),0,cudaMemcpyHostToDevice,stream );
    xdim0_update_halo_kernel4_plus_2_back_h = xdim0;
    cudaMemcpyToSymbolAsync( ydim0_update_halo_kernel4_plus_2_back, &ydim0, sizeof(int),0,cudaMemcpyHostToDevice,stream );
    ydim0_update_halo_kernel4_plus_2_back_h = ydim0;
    cudaMemcpyToSymbolAsync( xdim1_update_halo_kernel4_plus_2_back, &xdim1, sizeof(int),0,cudaMemcpyHostToDevice,stream );
    xdim1_update_halo_kernel4_plus_2_back_h = xdim1;
    cudaMemcpyToSymbolAsync( ydim1_update_halo_kernel4_plus_2_back, &ydim1, sizeof(int),0,cudaMemcpyHostToDevice,stream );
  ...
  ops_update_halo_kernel4_plus_2_back<<<grid, tblock, 0, stream >>> (  (double *)p_a[0], (double *)p_a[1],
...
void ops_par_loop_update_halo_kernel4_plus_2_back(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  desc->function = ops_par_loop_update_halo_kernel4_plus_2_back_execute;
  if (OPS_diags > 1) {
    ops_timing_realloc(126,"update_halo_kernel4_plus_2_back");
  }

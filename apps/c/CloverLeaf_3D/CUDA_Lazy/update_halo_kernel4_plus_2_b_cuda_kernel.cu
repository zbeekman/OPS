...
inline void update_halo_kernel4_plus_2_b_gpu(double *vol_flux_y, double *mass_flux_y, const int* fields) {
  if((*fields) & FIELD_VOL_FLUX_Y) vol_flux_y[OPS_ACC0(0,0,0)] = vol_flux_y[OPS_ACC0(-2,0,0)];
  if((*fields) & FIELD_MASS_FLUX_Y) mass_flux_y[OPS_ACC1(0,0,0)] = mass_flux_y[OPS_ACC1(-2,0,0)];
}
...
int size2 ){
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * xdim0_update_halo_kernel4_plus_2_b + idx_z * 1*1 * xdim0_update_halo_kernel4_plus_2_b * ydim0_update_halo_kernel4_plus_2_b;
  arg1 += idx_x * 1*1 + idx_y * 1*1 * xdim1_update_halo_kernel4_plus_2_b + idx_z * 1*1 * xdim1_update_halo_kernel4_plus_2_b * ydim1_update_halo_kernel4_plus_2_b;
...
void ops_par_loop_update_halo_kernel4_plus_2_b_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_update_halo_kernel4_plus_2_b_h || ydim0 != ydim0_update_halo_kernel4_plus_2_b_h || xdim1 != xdim1_update_halo_kernel4_plus_2_b_h || ydim1 != ydim1_update_halo_kernel4_plus_2_b_h) {
    cudaMemcpyToSymbolAsync( xdim0_update_halo_kernel4_plus_2_b, &xdim0, sizeof(int),0,cudaMemcpyHostToDevice,stream );
    xdim0_update_halo_kernel4_plus_2_b_h = xdim0;
    cudaMemcpyToSymbolAsync( ydim0_update_halo_kernel4_plus_2_b, &ydim0, sizeof(int),0,cudaMemcpyHostToDevice,stream );
    ydim0_update_halo_kernel4_plus_2_b_h = ydim0;
    cudaMemcpyToSymbolAsync( xdim1_update_halo_kernel4_plus_2_b, &xdim1, sizeof(int),0,cudaMemcpyHostToDevice,stream );
    xdim1_update_halo_kernel4_plus_2_b_h = xdim1;
    cudaMemcpyToSymbolAsync( ydim1_update_halo_kernel4_plus_2_b, &ydim1, sizeof(int),0,cudaMemcpyHostToDevice,stream );
  ...
  ops_update_halo_kernel4_plus_2_b<<<grid, tblock, 0, stream >>> (  (double *)p_a[0], (double *)p_a[1],

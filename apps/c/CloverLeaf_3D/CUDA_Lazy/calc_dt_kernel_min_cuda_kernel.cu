...
void ops_par_loop_calc_dt_kernel_min_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != xdim0_calc_dt_kernel_min_h || ydim0 != ydim0_calc_dt_kernel_min_h) {
    ...
    cudaMemcpyToSymbolAsync( ydim0_calc_dt_kernel_min, &ydim0, sizeof(int),0,cudaMemcpyHostToDevice,stream );
  ...
  ops_calc_dt_kernel_min<<<grid, tblock, nshared, stream >>> (  (double *)p_a[0], (double *)arg1.data_d,x_size, y_size, z_size);

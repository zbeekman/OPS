...
static bool isbuilt_update_halo_kernel3_plus_2_back = false;

void buildOpenCLKernels_update_halo_kernel3_plus_2_back(int xdim0, int ydim0,
                                                        int xdim1, int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel3_plus_2_back) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel3_plus_2_back.cl"};
    ...
    printf("Compiling update_halo_kernel3_plus_2_back %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel3_plus_2_back=%d  "
                           "-Dydim0_update_halo_kernel3_plus_2_back=%d  "
                           "-Dxdim1_update_halo_kernel3_plus_2_back=%d  "
                           "-Dydim1_update_halo_kernel3_plus_2_back=%d ",
    ...
    printf("compiling update_halo_kernel3_plus_2_back -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[114] = clCreateKernel(
        OPS_opencl_core.program, "ops_update_halo_kernel3_plus_2_back", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel3_plus_2_back = true;
  }
}

// host stub function
void ops_par_loop_update_halo_kernel3_plus_2_back(char const *name,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 3, range, 114))
    return;
#endif
  ...
  buildOpenCLKernels_update_halo_kernel3_plus_2_back(xdim0, ydim0, xdim1,
                                                     ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[114], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[114], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[114], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[114], 4, sizeof(cl_int),
                            (void *)&base1));

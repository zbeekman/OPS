...
void buildOpenCLKernels_update_halo_kernel2_xvel_minus_4_right(int xdim0,
  ...
  if (!isbuilt_update_halo_kernel2_xvel_minus_4_right) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_xvel_minus_4_right.cl"};
    ...
    printf("Compiling update_halo_kernel2_xvel_minus_4_right %d source -- "
           "start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_xvel_minus_4_right=%d  "
                           "-Dydim0_update_halo_kernel2_xvel_minus_4_right=%d  "
                           "-Dxdim1_update_halo_kernel2_xvel_minus_4_right=%d  "
                           "-Dydim1_update_halo_kernel2_xvel_minus_4_right=%d ",
    ...
    OPS_opencl_core.kernel[75] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_xvel_minus_4_right", &ret);
    clSafeCall(ret);
...
void ops_par_loop_update_halo_kernel2_xvel_minus_4_right(
  ...
  buildOpenCLKernels_update_halo_kernel2_xvel_minus_4_right(xdim0, ydim0, xdim1,
                                                            ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[75], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[75], 3, NULL,

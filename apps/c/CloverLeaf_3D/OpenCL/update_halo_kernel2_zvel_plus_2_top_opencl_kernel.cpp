...
static bool isbuilt_update_halo_kernel2_zvel_plus_2_top = false;

void buildOpenCLKernels_update_halo_kernel2_zvel_plus_2_top(int xdim0,
                                                            int ydim0,
                                                            int xdim1,
                                                            int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel2_zvel_plus_2_top) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_zvel_plus_2_top.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_zvel_plus_2_top %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_zvel_plus_2_top=%d  "
                           "-Dydim0_update_halo_kernel2_zvel_plus_2_top=%d  "
                           "-Dxdim1_update_halo_kernel2_zvel_plus_2_top=%d  "
                           "-Dydim1_update_halo_kernel2_zvel_plus_2_top=%d ",
    ...
    isbuilt_update_halo_kernel2_zvel_plus_2_top = true;
  }
}
...
void ops_par_loop_update_halo_kernel2_zvel_plus_2_top(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel2_zvel_plus_2_top(xdim0, ydim0, xdim1,
                                                         ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[96], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[96], 2, sizeof(cl_int),
                            (void *)arg2.data));

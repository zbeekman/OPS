...
void buildOpenCLKernels_update_halo_kernel3_plus_4_b(int xdim0, int ydim0,
                                                     int xdim1, int ydim1) {
  ...
  if (!isbuilt_update_halo_kernel3_plus_4_b) {
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           ...
                           "-Dydim0_update_halo_kernel3_plus_4_b=%d  "
                           "-Dxdim1_update_halo_kernel3_plus_4_b=%d  "
                           "-Dydim1_update_halo_kernel3_plus_4_b=%d ",
    ...
    OPS_opencl_core.kernel[107] = clCreateKernel(
...
void ops_par_loop_update_halo_kernel3_plus_4_b(char const *name,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 3, range, 107))
    return;
#endif
  ...
  buildOpenCLKernels_update_halo_kernel3_plus_4_b(xdim0, ydim0, xdim1, ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[107], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[107], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[107], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[107], 4, sizeof(cl_int),
                            (void *)&base1));

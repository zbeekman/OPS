...
static bool isbuilt_update_halo_kernel2_yvel_plus_4_right = false;

void buildOpenCLKernels_update_halo_kernel2_yvel_plus_4_right(int xdim0,
                                                              int ydim0,
                                                              int xdim1,
                                                              int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel2_yvel_plus_4_right) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_yvel_plus_4_right.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_yvel_plus_4_right %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_yvel_plus_4_right=%d  "
                           "-Dydim0_update_halo_kernel2_yvel_plus_4_right=%d  "
                           "-Dxdim1_update_halo_kernel2_yvel_plus_4_right=%d  "
                           "-Dydim1_update_halo_kernel2_yvel_plus_4_right=%d ",
    ...
    printf("compiling update_halo_kernel2_yvel_plus_4_right -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[87] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_yvel_plus_4_right", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel2_yvel_plus_4_right = true;
  }
}
...
void ops_par_loop_update_halo_kernel2_yvel_plus_4_right(
  ...
  buildOpenCLKernels_update_halo_kernel2_yvel_plus_4_right(xdim0, ydim0, xdim1,
                                                           ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[87], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[87], 2, sizeof(cl_int),
                            (void *)arg2.data));

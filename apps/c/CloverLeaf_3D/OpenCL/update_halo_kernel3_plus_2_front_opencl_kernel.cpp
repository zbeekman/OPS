...
void buildOpenCLKernels_update_halo_kernel3_plus_2_front(int xdim0, int ydim0,
                                                         int xdim1, int ydim1) {
  ...
  if (!isbuilt_update_halo_kernel3_plus_2_front) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel3_plus_2_front.cl"};
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel3_plus_2_front=%d  "
                           "-Dydim0_update_halo_kernel3_plus_2_front=%d  "
                           "-Dxdim1_update_halo_kernel3_plus_2_front=%d  "
                           "-Dydim1_update_halo_kernel3_plus_2_front=%d ",
    ...
    OPS_opencl_core.kernel[116] = clCreateKernel(
        OPS_opencl_core.program, "ops_update_halo_kernel3_plus_2_front", &ret);
    clSafeCall(ret);
...
// host stub function
void ops_par_loop_update_halo_kernel3_plus_2_front(char const *name,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 3, range, 116))
    return;
#endif
  ...
  buildOpenCLKernels_update_halo_kernel3_plus_2_front(xdim0, ydim0, xdim1,
                                                      ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[116], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[116], 3, NULL,

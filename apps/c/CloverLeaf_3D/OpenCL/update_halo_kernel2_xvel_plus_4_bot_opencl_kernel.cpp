...
static bool isbuilt_update_halo_kernel2_xvel_plus_4_bot = false;

void buildOpenCLKernels_update_halo_kernel2_xvel_plus_4_bot(int xdim0,
                                                            int ydim0,
                                                            int xdim1,
                                                            int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel2_xvel_plus_4_bot) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_xvel_plus_4_bot.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_xvel_plus_4_bot %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_xvel_plus_4_bot=%d  "
                           "-Dydim0_update_halo_kernel2_xvel_plus_4_bot=%d  "
                           "-Dxdim1_update_halo_kernel2_xvel_plus_4_bot=%d  "
                           "-Dydim1_update_halo_kernel2_xvel_plus_4_bot=%d ",
    ...
    printf("compiling update_halo_kernel2_xvel_plus_4_bot -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[69] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_xvel_plus_4_bot", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel2_xvel_plus_4_bot = true;
  }
}
...
void ops_par_loop_update_halo_kernel2_xvel_plus_4_bot(char const *name,
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[69], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[69], 3, NULL,

...
static bool isbuilt_update_halo_kernel2_zvel_minus_2_back = false;

void buildOpenCLKernels_update_halo_kernel2_zvel_minus_2_back(int xdim0,
                                                              int ydim0,
                                                              int xdim1,
                                                              int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel2_zvel_minus_2_back) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_zvel_minus_2_back.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_zvel_minus_2_back %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_zvel_minus_2_back=%d  "
                           "-Dydim0_update_halo_kernel2_zvel_minus_2_back=%d  "
                           "-Dxdim1_update_halo_kernel2_zvel_minus_2_back=%d  "
                           "-Dydim1_update_halo_kernel2_zvel_minus_2_back=%d ",
    ...
    printf("compiling update_halo_kernel2_zvel_minus_2_back -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[102] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_zvel_minus_2_back", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel2_zvel_minus_2_back = true;
  }
}
...
void ops_par_loop_update_halo_kernel2_zvel_minus_2_back(
  ...
  buildOpenCLKernels_update_halo_kernel2_zvel_minus_2_back(xdim0, ydim0, xdim1,
                                                           ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[102], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[102], 3, NULL,

...
static bool isbuilt_update_halo_kernel4_plus_4_front = false;

void buildOpenCLKernels_update_halo_kernel4_plus_4_front(int xdim0, int ydim0,
                                                         int xdim1, int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel4_plus_4_front) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel4_plus_4_front.cl"};
    ...
    printf("Compiling update_halo_kernel4_plus_4_front %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel4_plus_4_front=%d  "
                           "-Dydim0_update_halo_kernel4_plus_4_front=%d  "
                           "-Dxdim1_update_halo_kernel4_plus_4_front=%d  "
                           "-Dydim1_update_halo_kernel4_plus_4_front=%d ",
    ...
    printf("compiling update_halo_kernel4_plus_4_front -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[127] = clCreateKernel(
        OPS_opencl_core.program, "ops_update_halo_kernel4_plus_4_front", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel4_plus_4_front = true;
  }
}
...
void ops_par_loop_update_halo_kernel4_plus_4_front(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel4_plus_4_front(xdim0, ydim0, xdim1,
                                                      ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[127], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[127], 7, sizeof(cl_int),
                            (void *)&z_size));

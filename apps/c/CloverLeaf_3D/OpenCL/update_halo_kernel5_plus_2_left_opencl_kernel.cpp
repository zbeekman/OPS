...
static bool isbuilt_update_halo_kernel5_plus_2_left = false;

void buildOpenCLKernels_update_halo_kernel5_plus_2_left(int xdim0, int ydim0,
                                                        int xdim1, int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel5_plus_2_left) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel5_plus_2_left.cl"};
    ...
    printf("Compiling update_halo_kernel5_plus_2_left %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel5_plus_2_left=%d  "
                           "-Dydim0_update_halo_kernel5_plus_2_left=%d  "
                           "-Dxdim1_update_halo_kernel5_plus_2_left=%d  "
                           "-Dydim1_update_halo_kernel5_plus_2_left=%d ",
    ...
    printf("compiling update_halo_kernel5_plus_2_left -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[134] = clCreateKernel(
        OPS_opencl_core.program, "ops_update_halo_kernel5_plus_2_left", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel5_plus_2_left = true;
  }
}
...
void ops_par_loop_update_halo_kernel5_plus_2_left(char const *name,
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[134], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[134], 7, sizeof(cl_int),
                            (void *)&z_size));

...
static bool isbuilt_update_halo_kernel2_yvel_minus_4_bot = false;

void buildOpenCLKernels_update_halo_kernel2_yvel_minus_4_bot(int xdim0,
                                                             int ydim0,
                                                             int xdim1,
                                                             int ydim1) {
  ...
  if (!isbuilt_update_halo_kernel2_yvel_minus_4_bot) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_yvel_minus_4_bot.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_yvel_minus_4_bot %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_yvel_minus_4_bot=%d  "
                           "-Dydim0_update_halo_kernel2_yvel_minus_4_bot=%d  "
                           "-Dxdim1_update_halo_kernel2_yvel_minus_4_bot=%d  "
                           "-Dydim1_update_halo_kernel2_yvel_minus_4_bot=%d ",
    ...
    printf("compiling update_halo_kernel2_yvel_minus_4_bot -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[81] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_yvel_minus_4_bot", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel2_yvel_minus_4_bot = true;
  }
}
...
void ops_par_loop_update_halo_kernel2_yvel_minus_4_bot(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel2_yvel_minus_4_bot(xdim0, ydim0, xdim1,
                                                          ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[81], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[81], 3, NULL,

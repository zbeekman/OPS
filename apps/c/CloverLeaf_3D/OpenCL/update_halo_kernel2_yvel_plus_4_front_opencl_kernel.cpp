...
static bool isbuilt_update_halo_kernel2_yvel_plus_4_front = false;

void buildOpenCLKernels_update_halo_kernel2_yvel_plus_4_front(int xdim0,
                                                              int ydim0,
                                                              int xdim1,
                                                              int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel2_yvel_plus_4_front) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_yvel_plus_4_front.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_yvel_plus_4_front %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_yvel_plus_4_front=%d  "
                           "-Dydim0_update_halo_kernel2_yvel_plus_4_front=%d  "
                           "-Dxdim1_update_halo_kernel2_yvel_plus_4_front=%d  "
                           "-Dydim1_update_halo_kernel2_yvel_plus_4_front=%d ",
    ...
    OPS_opencl_core.kernel[91] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_yvel_plus_4_front", &ret);
    clSafeCall(ret);
...
void ops_par_loop_update_halo_kernel2_yvel_plus_4_front(
  ...
  buildOpenCLKernels_update_halo_kernel2_yvel_plus_4_front(xdim0, ydim0, xdim1,
                                                           ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[91], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[91], 3, NULL,

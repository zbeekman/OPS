...
static bool isbuilt_update_halo_kernel2_xvel_plus_2_top = false;

void buildOpenCLKernels_update_halo_kernel2_xvel_plus_2_top(int xdim0,
                                                            int ydim0,
                                                            int xdim1,
                                                            int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel2_xvel_plus_2_top) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel2_xvel_plus_2_top.cl"};
    ...
    printf(
        "Compiling update_halo_kernel2_xvel_plus_2_top %d source -- start \n",
        OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel2_xvel_plus_2_top=%d  "
                           "-Dydim0_update_halo_kernel2_xvel_plus_2_top=%d  "
                           "-Dxdim1_update_halo_kernel2_xvel_plus_2_top=%d  "
                           "-Dydim1_update_halo_kernel2_xvel_plus_2_top=%d ",
    ...
    printf("compiling update_halo_kernel2_xvel_plus_2_top -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[72] =
        clCreateKernel(OPS_opencl_core.program,
                       "ops_update_halo_kernel2_xvel_plus_2_top", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel2_xvel_plus_2_top = true;
  }
}
...
void ops_par_loop_update_halo_kernel2_xvel_plus_2_top(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel2_xvel_plus_2_top(xdim0, ydim0, xdim1,
                                                         ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[72], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[72], 3, NULL,

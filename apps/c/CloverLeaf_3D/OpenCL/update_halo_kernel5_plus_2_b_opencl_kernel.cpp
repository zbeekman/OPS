...
static bool isbuilt_update_halo_kernel5_plus_2_b = false;

void buildOpenCLKernels_update_halo_kernel5_plus_2_b(int xdim0, int ydim0,
                                                     int xdim1, int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel5_plus_2_b) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel5_plus_2_b.cl"};
    ...
    printf("Compiling update_halo_kernel5_plus_2_b %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel5_plus_2_b=%d  "
                           "-Dydim0_update_halo_kernel5_plus_2_b=%d  "
                           "-Dxdim1_update_halo_kernel5_plus_2_b=%d  "
                           "-Dydim1_update_halo_kernel5_plus_2_b=%d ",
    ...
    isbuilt_update_halo_kernel5_plus_2_b = true;
  }
}
...
void ops_par_loop_update_halo_kernel5_plus_2_b(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel5_plus_2_b(xdim0, ydim0, xdim1, ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[132], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[132], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[132], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[132], 4, sizeof(cl_int),
                            (void *)&base1));

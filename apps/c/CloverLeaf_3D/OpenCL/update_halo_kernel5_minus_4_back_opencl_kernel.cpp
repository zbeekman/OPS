...
static bool isbuilt_update_halo_kernel5_minus_4_back = false;

void buildOpenCLKernels_update_halo_kernel5_minus_4_back(int xdim0, int ydim0,
                                                         int xdim1, int ydim1) {
  ...
  if (!isbuilt_update_halo_kernel5_minus_4_back) {
    ...
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel5_minus_4_back.cl"};
    ...
    printf("Compiling update_halo_kernel5_minus_4_back %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel5_minus_4_back=%d  "
                           "-Dydim0_update_halo_kernel5_minus_4_back=%d  "
                           "-Dxdim1_update_halo_kernel5_minus_4_back=%d  "
                           "-Dydim1_update_halo_kernel5_minus_4_back=%d ",
    ...
    printf("compiling update_halo_kernel5_minus_4_back -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[137] = clCreateKernel(
        OPS_opencl_core.program, "ops_update_halo_kernel5_minus_4_back", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel5_minus_4_back = true;
  }
}
...
void ops_par_loop_update_halo_kernel5_minus_4_back(char const *name,
  ...
  buildOpenCLKernels_update_halo_kernel5_minus_4_back(xdim0, ydim0, xdim1,
                                                      ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 0, sizeof(cl_mem),
                            (void *)&arg0.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 5, sizeof(cl_int),
                            (void *)&x_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 6, sizeof(cl_int),
                            (void *)&y_size));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[137], 7, sizeof(cl_int),
                            (void *)&z_size));

  // call/enque opencl kernel wrapper function
  clSafeCall(clEnqueueNDRangeKernel(
      OPS_opencl_core.command_queue, OPS_opencl_core.kernel[137], 3, NULL,

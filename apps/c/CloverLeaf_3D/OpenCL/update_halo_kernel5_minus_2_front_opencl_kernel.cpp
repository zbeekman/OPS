...
static bool isbuilt_update_halo_kernel5_minus_2_front = false;

void buildOpenCLKernels_update_halo_kernel5_minus_2_front(int xdim0, int ydim0,
                                                          int xdim1,
                                                          int ydim1) {

  // int ocl_fma = OCL_FMA;
  if (!isbuilt_update_halo_kernel5_minus_2_front) {
    buildOpenCLKernels();
    // clSafeCall( clUnloadCompiler() );
    cl_int ret;
    char *source_filename[1] = {
        (char *)"./OpenCL/update_halo_kernel5_minus_2_front.cl"};
    ...
    printf("Compiling update_halo_kernel5_minus_2_front %d source -- start \n",
           OCL_FMA);
    ...
    if (pPath != NULL)
      if (OCL_FMA)
        sprintf(buildOpts, "-cl-mad-enable -DOCL_FMA -I%s/c/include "
                           "-DOPS_WARPSIZE=%d  "
                           "-Dxdim0_update_halo_kernel5_minus_2_front=%d  "
                           "-Dydim0_update_halo_kernel5_minus_2_front=%d  "
                           "-Dxdim1_update_halo_kernel5_minus_2_front=%d  "
                           "-Dydim1_update_halo_kernel5_minus_2_front=%d ",
    ...
    printf("compiling update_halo_kernel5_minus_2_front -- done\n");

    // Create the OpenCL kernel
    OPS_opencl_core.kernel[140] = clCreateKernel(
        OPS_opencl_core.program, "ops_update_halo_kernel5_minus_2_front", &ret);
    clSafeCall(ret);

    isbuilt_update_halo_kernel5_minus_2_front = true;
  }
}
...
void ops_par_loop_update_halo_kernel5_minus_2_front(char const *name,
...
#ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args, 3, range, 140))
    return;
#endif
  ...
  buildOpenCLKernels_update_halo_kernel5_minus_2_front(xdim0, ydim0, xdim1,
                                                       ydim1);
  ...
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[140], 1, sizeof(cl_mem),
                            (void *)&arg1.data_d));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[140], 2, sizeof(cl_int),
                            (void *)arg2.data));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[140], 3, sizeof(cl_int),
                            (void *)&base0));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[140], 4, sizeof(cl_int),
                            (void *)&base1));
  clSafeCall(clSetKernelArg(OPS_opencl_core.kernel[140], 5, sizeof(cl_int),
                            (void *)&x_size));
